...
3. После запуска создаются **два** файла с одинаковыми данными — импортируйте в NocoBase **любой удобный формат**:
   ...
   Исходник **`Товары.xlsx`** в корне — это *ваша* таблица; скрипт **не** «переводит в xlsx», а наоборот: **из вашего xlsx делает нормализованные csv+xlsx** для коллекции `products`. Если нужен только Excel — берите `products-nocobase.xlsx`.
