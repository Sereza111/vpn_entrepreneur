...
- **Круговая / столбцы**: сумма `amount` по полю `role` (перед этим приведите все строки к одной валюте или стройте два ряда RUB / USD).
- **Динамика**: при нескольких записях с разными `effectiveFrom` — линия «итого инфра в ₽/мес» по времени (вручную или через SQL-блок, если включён).

### Выручка и безубыток (orders)

- **Выручка**: сумма `orders.amount` по `paidAt` (фильтр `status=paid`), группировка по дню.
- **Безубыток (упрощённо)**: в одном отчёте сравните за месяц:
  - агрегат **сумма orders.amount** (RUB);
  - **сумма эквивалента infra_costs в ₽/мес** (см. выше).
  Точная маржа с учётом комиссий платёжки — вне NocoBase или отдельным полем в заказе.
...
Точные шаги зависят от версии NocoBase и включённых плагинов визуализации — используйте встроенный конструктор (Builder: коллекция → меры/измерения → **Выполнить запрос**).
