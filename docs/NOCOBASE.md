...
3. После запуска создаются **два** файла с одинаковыми данными — импортируйте в NocoBase **любой удобный формат**:
   - **`import/products-nocobase.csv`** (UTF-8 с BOM);
   - **`import/products-nocobase.xlsx`** (Excel).

   Исходник **`Товары.xlsx`** в корне — это *ваша* таблица; скрипт **из вашего xlsx** собирает **`products-nocobase.csv`** и **`products-nocobase.xlsx`** с заголовками как у экспорта NocoBase: **`ID`, `code`, `Целое число`, `productType`, `title`, `sortOrder`, `active`** (поле **`grantDays`** в экспорте называется **«Целое число»**). Колонка **`serverId`** в этот шаблон не входит — при необходимости укажите в таблице после импорта.
